...
- Make HTTP requests to: `http://localhost:3000/mockserver{baseUrl}{path}`
- Example: `http://localhost:3000/mockserver/api/v1/users`

## API URL Structure

Your fake APIs follow this pattern:
```
{BASE_URL}/mockserver/{projectName}{projectBaseUrl}{endpointPath}
```
...
- Endpoint Path: `/users`
- Full URL: `http://localhost:3000/mockserver/my-blog-api/api/v1/users`
...
curl http://localhost:3000/mockserver/my-api/api/v1/users

# With token (200 Success)
curl -H "Authorization: Bearer abcd-1234-efgh-5678" \
     http://localhost:3000/mockserver/my-api/api/v1/users
```
...
const users = await fetch('http://localhost:3000/mockserver/user-management-api/api/v1/users');

// Create a user
const newUser = await fetch('http://localhost:3000/mockserver/user-management-api/api/v1/users', {
  method: 'POST',
